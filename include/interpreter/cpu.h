#ifndef CPU_H
#define CPU_H

#include "interpreter/registers.h"
#include "interpreter/flags.h"
#include "interpreter/x87.h"
#include "instructions.h"

namespace x64 {

    class VM;
    class Mmu;
    struct X86Instruction;

    class Cpu {
    public:
        Cpu(VM* vm, Mmu* mmu) : vm_(vm), mmu_(mmu) { }

    private:
        friend class VM;
        
        VM* vm_;
        Mmu* mmu_;
        Flags flags_;
        Registers regs_;
        X87Fpu x87fpu_;

        struct FPUState {
            u16 fcw;
            u16 fsw;
            u32 unused0;
            u64 unused1;
            u128 fpu1;
            u128 st0;
            u128 st1;
            u128 st2;
            u128 st3;
            u128 st4;
            u128 st5;
            u128 st6;
            u128 st7;
            u128 xmm0;
            u128 xmm1;
            u128 xmm2;
            u128 xmm3;
            u128 xmm4;
            u128 xmm5;
            u128 xmm6;
            u128 xmm7;
            u128 xmm8;
            u128 xmm9;
            u128 xmm10;
            u128 xmm11;
            u128 xmm12;
            u128 xmm13;
            u128 xmm14;
            u128 xmm15;
            u128 reserved0;
            u128 reserved1;
            u128 reserved2;
            u128 available0;
            u128 available1;
            u128 available2;
        };

        static_assert(sizeof(FPUState) == 512, "FPUState must be 512 bytes");

        FPUState getFpuState() const;
        void setFpuState(const FPUState&);

        u8  get(R8 reg) const  { return regs_.get(reg); }
        u16 get(R16 reg) const { return regs_.get(reg); }
        u32 get(R32 reg) const { return regs_.get(reg); }
        u64 get(R64 reg) const { return regs_.get(reg); }
        Xmm get(RSSE reg) const { return regs_.get(reg); }

        template<typename T>
        T  get(Imm immediate) const;

        u8  get(Ptr8 ptr) const;
        u16 get(Ptr16 ptr) const;
        u32 get(Ptr32 ptr) const;
        u64 get(Ptr64 ptr) const;
        f80 get(Ptr80 ptr) const;
        Xmm get(Ptr128 ptr) const;

        u64 resolve(B addr) const { return regs_.resolve(addr); }
        u64 resolve(BD addr) const { return regs_.resolve(addr); }
        u64 resolve(BIS addr) const { return regs_.resolve(addr); }
        u64 resolve(ISD addr) const { return regs_.resolve(addr); }
        u64 resolve(BISD addr) const { return regs_.resolve(addr); }
        u64 resolve(SO addr) const { return regs_.resolve(addr); }

        template<Size size, typename Enc>
        SPtr<size> resolve(Addr<size, Enc> addr) const { return regs_.resolve(addr); }

        Ptr8 resolve(const M8& m8) const { return regs_.resolve(m8); }
        Ptr16 resolve(const M16& m16) const { return regs_.resolve(m16); }
        Ptr32 resolve(const M32& m32) const { return regs_.resolve(m32); }
        Ptr64 resolve(const M64& m64) const { return regs_.resolve(m64); }
        Ptr80 resolve(const M80& m80) const { return regs_.resolve(m80); }
        Ptr128 resolve(const MSSE& msse) const { return regs_.resolve(msse); }
        Ptr224 resolve(const M224& m224) const { return regs_.resolve(m224); }

        void set(R8 reg, u8 value) { regs_.set(reg, value); }
        void set(R16 reg, u16 value) { regs_.set(reg, value); }
        void set(R32 reg, u32 value) { regs_.set(reg, value); }
        void set(R64 reg, u64 value) { regs_.set(reg, value); }
        void set(RSSE reg, Xmm value) { regs_.set(reg, value); }

        void set(Ptr8 ptr, u8 value);
        void set(Ptr16 ptr, u16 value);
        void set(Ptr32 ptr, u32 value);
        void set(Ptr64 ptr, u64 value);
        void set(Ptr80 ptr, f80 value);
        void set(Ptr128 ptr, Xmm value);

        void push8(u8 value);
        void push16(u16 value);
        void push32(u32 value);
        void push64(u64 value);
        u8 pop8();
        u16 pop16();
        u32 pop32();
        u64 pop64();

        template<typename Dst>
        void execSet(Cond cond, Dst dst);

        template<typename Dst, typename Src>
        void execCmovImpl(Cond cond, Dst dst, Src src);

        void execFcmovImpl(Cond cond, ST dst, ST src);

        template<typename Dst>
        void execCmpxchg32Impl(Dst dst, u32 src);

        template<typename Dst>
        void execCmpxchg64Impl(Dst dst, u64 src);

    public:
        void exec(const Add<R8, R8>&);
        void exec(const Add<R8, Imm>&);
        void exec(const Add<R8, M8>&);
        void exec(const Add<M8, R8>&);
        void exec(const Add<M8, Imm>&);
        void exec(const Add<R16, R16>&);
        void exec(const Add<R16, Imm>&);
        void exec(const Add<R16, M16>&);
        void exec(const Add<M16, R16>&);
        void exec(const Add<M16, Imm>&);
        void exec(const Add<R32, R32>&);
        void exec(const Add<R32, Imm>&);
        void exec(const Add<R32, M32>&);
        void exec(const Add<M32, R32>&);
        void exec(const Add<M32, Imm>&);
        void exec(const Add<R64, R64>&);
        void exec(const Add<R64, Imm>&);
        void exec(const Add<R64, M64>&);
        void exec(const Add<M64, R64>&);
        void exec(const Add<M64, Imm>&);

        void exec(const Adc<R32, R32>&);
        void exec(const Adc<R32, Imm>&);
        void exec(const Adc<R32, M32>&);
        void exec(const Adc<M32, R32>&);
        void exec(const Adc<M32, Imm>&);
        void exec(const Adc<R64, R64>&);
        void exec(const Adc<R64, Imm>&);
        void exec(const Adc<R64, M64>&);
        void exec(const Adc<M64, R64>&);
        void exec(const Adc<M64, Imm>&);

        void exec(const Sub<R8, R8>&);
        void exec(const Sub<R8, Imm>&);
        void exec(const Sub<R8, M8>&);
        void exec(const Sub<M8, R8>&);
        void exec(const Sub<M8, Imm>&);
        void exec(const Sub<R16, R16>&);
        void exec(const Sub<R16, Imm>&);
        void exec(const Sub<R16, M16>&);
        void exec(const Sub<M16, R16>&);
        void exec(const Sub<M16, Imm>&);
        void exec(const Sub<R32, R32>&);
        void exec(const Sub<R32, Imm>&);
        void exec(const Sub<R32, M32>&);
        void exec(const Sub<M32, R32>&);
        void exec(const Sub<M32, Imm>&);
        void exec(const Sub<R64, R64>&);
        void exec(const Sub<R64, Imm>&);
        void exec(const Sub<R64, M64>&);
        void exec(const Sub<M64, R64>&);
        void exec(const Sub<M64, Imm>&);

        void exec(const Sbb<R8, R8>&);
        void exec(const Sbb<R8, Imm>&);
        void exec(const Sbb<R8, M8>&);
        void exec(const Sbb<M8, R8>&);
        void exec(const Sbb<M8, Imm>&);
        void exec(const Sbb<R16, R16>&);
        void exec(const Sbb<R16, Imm>&);
        void exec(const Sbb<R16, M16>&);
        void exec(const Sbb<M16, R16>&);
        void exec(const Sbb<M16, Imm>&);
        void exec(const Sbb<R32, R32>&);
        void exec(const Sbb<R32, Imm>&);
        void exec(const Sbb<R32, M32>&);
        void exec(const Sbb<M32, R32>&);
        void exec(const Sbb<M32, Imm>&);
        void exec(const Sbb<R64, R64>&);
        void exec(const Sbb<R64, Imm>&);
        void exec(const Sbb<R64, M64>&);
        void exec(const Sbb<M64, R64>&);
        void exec(const Sbb<M64, Imm>&);

        void exec(const Neg<R8>&);
        void exec(const Neg<M8>&);
        void exec(const Neg<R16>&);
        void exec(const Neg<M16>&);
        void exec(const Neg<R32>&);
        void exec(const Neg<M32>&);
        void exec(const Neg<R64>&);
        void exec(const Neg<M64>&);

        void exec(const Mul<R32>&);
        void exec(const Mul<M32>&);
        void exec(const Mul<R64>&);
        void exec(const Mul<M64>&);

        void exec(const Imul1<R32>&);
        void exec(const Imul1<M32>&);
        void exec(const Imul2<R32, R32>&);
        void exec(const Imul2<R32, M32>&);
        void exec(const Imul3<R32, R32, Imm>&);
        void exec(const Imul3<R32, M32, Imm>&);
        void exec(const Imul1<R64>&);
        void exec(const Imul1<M64>&);
        void exec(const Imul2<R64, R64>&);
        void exec(const Imul2<R64, M64>&);
        void exec(const Imul3<R64, R64, Imm>&);
        void exec(const Imul3<R64, M64, Imm>&);

        void exec(const Div<R32>&);
        void exec(const Div<M32>&);
        void exec(const Div<R64>&);
        void exec(const Div<M64>&);

        void exec(const Idiv<R32>&);
        void exec(const Idiv<M32>&);
        void exec(const Idiv<R64>&);
        void exec(const Idiv<M64>&);

        void exec(const And<R8, R8>&);
        void exec(const And<R8, Imm>&);
        void exec(const And<R8, M8>&);
        void exec(const And<M8, R8>&);
        void exec(const And<M8, Imm>&);
        void exec(const And<R16, Imm>&);
        void exec(const And<R16, R16>&);
        void exec(const And<R16, M16>&);
        void exec(const And<M16, Imm>&);
        void exec(const And<M16, R16>&);
        void exec(const And<R32, R32>&);
        void exec(const And<R32, Imm>&);
        void exec(const And<R32, M32>&);
        void exec(const And<M32, R32>&);
        void exec(const And<M32, Imm>&);
        void exec(const And<R64, R64>&);
        void exec(const And<R64, Imm>&);
        void exec(const And<R64, M64>&);
        void exec(const And<M64, R64>&);
        void exec(const And<M64, Imm>&);

        void exec(const Or<R8, R8>&);
        void exec(const Or<R8, Imm>&);
        void exec(const Or<R8, M8>&);
        void exec(const Or<M8, R8>&);
        void exec(const Or<M8, Imm>&);
        void exec(const Or<R16, R16>&);
        void exec(const Or<R16, Imm>&);
        void exec(const Or<R16, M16>&);
        void exec(const Or<M16, R16>&);
        void exec(const Or<R32, R32>&);
        void exec(const Or<R32, Imm>&);
        void exec(const Or<R32, M32>&);
        void exec(const Or<M32, R32>&);
        void exec(const Or<M32, Imm>&);
        void exec(const Or<R64, R64>&);
        void exec(const Or<R64, Imm>&);
        void exec(const Or<R64, M64>&);
        void exec(const Or<M64, R64>&);
        void exec(const Or<M64, Imm>&);

        void exec(const Xor<R8, R8>&);
        void exec(const Xor<R8, Imm>&);
        void exec(const Xor<R8, M8>&);
        void exec(const Xor<M8, Imm>&);
        void exec(const Xor<R16, Imm>&);
        void exec(const Xor<R32, R32>&);
        void exec(const Xor<R32, Imm>&);
        void exec(const Xor<R32, M32>&);
        void exec(const Xor<M32, R32>&);
        void exec(const Xor<R64, R64>&);
        void exec(const Xor<R64, Imm>&);
        void exec(const Xor<R64, M64>&);
        void exec(const Xor<M64, R64>&);

        void exec(const Not<R8>&);
        void exec(const Not<M8>&);
        void exec(const Not<R16>&);
        void exec(const Not<M16>&);
        void exec(const Not<R32>&);
        void exec(const Not<M32>&);
        void exec(const Not<R64>&);
        void exec(const Not<M64>&);

        void exec(const Xchg<R16, R16>&);
        void exec(const Xchg<R32, R32>&);
        void exec(const Xchg<M32, R32>&);
        void exec(const Xchg<R64, R64>&);
        void exec(const Xchg<M64, R64>&);

        void exec(const Xadd<R16, R16>&);
        void exec(const Xadd<R32, R32>&);
        void exec(const Xadd<M32, R32>&);
        void exec(const Xadd<R64, R64>&);
        void exec(const Xadd<M64, R64>&);

        void exec(const Mov<R8, R8>&);
        void exec(const Mov<R8, Imm>&);
        void exec(const Mov<R8, M8>&);
        void exec(const Mov<M8, R8>&);
        void exec(const Mov<M8, Imm>&);
        void exec(const Mov<R16, R16>&);
        void exec(const Mov<R16, Imm>&);
        void exec(const Mov<R16, M16>&);
        void exec(const Mov<M16, R16>&);
        void exec(const Mov<M16, Imm>&);
        void exec(const Mov<R32, R32>&);
        void exec(const Mov<R32, Imm>&);
        void exec(const Mov<R32, M32>&);
        void exec(const Mov<M32, R32>&);
        void exec(const Mov<M32, Imm>&);
        void exec(const Mov<R64, R64>&);
        void exec(const Mov<R64, Imm>&);
        void exec(const Mov<R64, M64>&);
        void exec(const Mov<M64, R64>&);
        void exec(const Mov<M64, Imm>&);
        void exec(const Mov<RSSE, RSSE>&);
        void exec(const Mov<RSSE, MSSE>&);
        void exec(const Mov<MSSE, RSSE>&);

        void exec(const Movsx<R32, R8>&);
        void exec(const Movsx<R32, M8>&);
        void exec(const Movsx<R64, R8>&);
        void exec(const Movsx<R64, M8>&);
        void exec(const Movsx<R32, R16>&);
        void exec(const Movsx<R32, M16>&);
        void exec(const Movsx<R64, R16>&);
        void exec(const Movsx<R64, M16>&);
        void exec(const Movsx<R32, R32>&);
        void exec(const Movsx<R32, M32>&);
        void exec(const Movsx<R64, R32>&);
        void exec(const Movsx<R64, M32>&);

        void exec(const Movzx<R16, R8>&);
        void exec(const Movzx<R32, R8>&);
        void exec(const Movzx<R32, R16>&);
        void exec(const Movzx<R32, M8>&);
        void exec(const Movzx<R32, M16>&);

        void exec(const Lea<R32, B>&);
        void exec(const Lea<R32, BD>&);
        void exec(const Lea<R32, BIS>&);
        void exec(const Lea<R32, ISD>&);
        void exec(const Lea<R32, BISD>&);
        void exec(const Lea<R64, B>&);
        void exec(const Lea<R64, BD>&);
        void exec(const Lea<R64, BIS>&);
        void exec(const Lea<R64, ISD>&);
        void exec(const Lea<R64, BISD>&);

        void exec(const Push<Imm>&);
        void exec(const Push<R32>&);
        void exec(const Push<M32>&);
        void exec(const Push<R64>&);
        void exec(const Push<M64>&);

        void exec(const Pop<R32>&);
        void exec(const Pop<R64>&);

        void exec(const CallDirect&);
        void exec(const CallIndirect<R32>&);
        void exec(const CallIndirect<M32>&);
        void exec(const CallIndirect<R64>&);
        void exec(const CallIndirect<M64>&);
        void exec(const Ret<>&);
        void exec(const Ret<Imm>&);

        void exec(const Leave&);
        void exec(const Halt&);
        void exec(const Nop&);
        void exec(const Ud2&);
        void exec(const Syscall&);
        void exec(const NotParsed&);
        void exec(const Unknown&);

        void exec(const Cdq&);
        void exec(const Cqo&);

        void exec(const Inc<R8>&);
        void exec(const Inc<M8>&);
        void exec(const Inc<M16>&);
        void exec(const Inc<R32>&);
        void exec(const Inc<M32>&);
        void exec(const Inc<R64>&);
        void exec(const Inc<M64>&);

        void exec(const Dec<R8>&);
        void exec(const Dec<M16>&);
        void exec(const Dec<R32>&);
        void exec(const Dec<M32>&);

        void exec(const Shr<R8, R8>&);
        void exec(const Shr<R8, Imm>&);
        void exec(const Shr<R16, R8>&);
        void exec(const Shr<R16, Imm>&);
        void exec(const Shr<R32, R8>&);
        void exec(const Shr<M32, R8>&);
        void exec(const Shr<R32, Imm>&);
        void exec(const Shr<M32, Imm>&);
        void exec(const Shr<R64, R8>&);
        void exec(const Shr<M64, R8>&);
        void exec(const Shr<R64, Imm>&);
        void exec(const Shr<M64, Imm>&);

        void exec(const Shl<R8, R8>&);
        void exec(const Shl<R8, Imm>&);
        void exec(const Shl<R16, R8>&);
        void exec(const Shl<R16, Imm>&);
        void exec(const Shl<R32, R8>&);
        void exec(const Shl<M32, R8>&);
        void exec(const Shl<R32, Imm>&);
        void exec(const Shl<M32, Imm>&);
        void exec(const Shl<R64, R8>&);
        void exec(const Shl<M64, R8>&);
        void exec(const Shl<R64, Imm>&);
        void exec(const Shl<M64, Imm>&);

        void exec(const Shld<R32, R32, R8>&);
        void exec(const Shld<R32, R32, Imm>&);
        void exec(const Shld<R64, R64, R8>&);
        void exec(const Shld<R64, R64, Imm>&);

        void exec(const Shrd<R32, R32, R8>&);
        void exec(const Shrd<R32, R32, Imm>&);
        void exec(const Shrd<R64, R64, R8>&);
        void exec(const Shrd<R64, R64, Imm>&);

        void exec(const Sar<R32, R8>&);
        void exec(const Sar<R32, Imm>&);
        void exec(const Sar<M32, Imm>&);
        void exec(const Sar<R64, R8>&);
        void exec(const Sar<R64, Imm>&);
        void exec(const Sar<M64, Imm>&);

        void exec(const Rol<R8, R8>&);
        void exec(const Rol<R8, Imm>&);
        void exec(const Rol<M8, Imm>&);
        void exec(const Rol<R16, R8>&);
        void exec(const Rol<R16, Imm>&);
        void exec(const Rol<M16, Imm>&);
        void exec(const Rol<R32, R8>&);
        void exec(const Rol<R32, Imm>&);
        void exec(const Rol<M32, Imm>&);
        void exec(const Rol<R64, R8>&);
        void exec(const Rol<R64, Imm>&);
        void exec(const Rol<M64, Imm>&);

        void exec(const Ror<R8, R8>&);
        void exec(const Ror<R8, Imm>&);
        void exec(const Ror<M8, Imm>&);
        void exec(const Ror<R16, R8>&);
        void exec(const Ror<R16, Imm>&);
        void exec(const Ror<M16, Imm>&);
        void exec(const Ror<R32, R8>&);
        void exec(const Ror<R32, Imm>&);
        void exec(const Ror<M32, Imm>&);
        void exec(const Ror<R64, R8>&);
        void exec(const Ror<R64, Imm>&);
        void exec(const Ror<M64, Imm>&);

        void exec(const Tzcnt<R16, R16>&);
        void exec(const Tzcnt<R16, M16>&);
        void exec(const Tzcnt<R32, R32>&);
        void exec(const Tzcnt<R32, M32>&);
        void exec(const Tzcnt<R64, R64>&);
        void exec(const Tzcnt<R64, M64>&);

        void exec(const Bt<R16, R16>&);
        void exec(const Bt<R16, Imm>&);
        void exec(const Bt<R32, R32>&);
        void exec(const Bt<R32, Imm>&);
        void exec(const Bt<R64, R64>&);
        void exec(const Bt<R64, Imm>&);
        void exec(const Bt<M16, R16>&);
        void exec(const Bt<M16, Imm>&);
        void exec(const Bt<M32, R32>&);
        void exec(const Bt<M32, Imm>&);
        void exec(const Bt<M64, R64>&);
        void exec(const Bt<M64, Imm>&);

        void exec(const Btr<R16, R16>&);
        void exec(const Btr<R16, Imm>&);
        void exec(const Btr<R32, R32>&);
        void exec(const Btr<R32, Imm>&);
        void exec(const Btr<R64, R64>&);
        void exec(const Btr<R64, Imm>&);
        void exec(const Btr<M16, R16>&);
        void exec(const Btr<M16, Imm>&);
        void exec(const Btr<M32, R32>&);
        void exec(const Btr<M32, Imm>&);
        void exec(const Btr<M64, R64>&);
        void exec(const Btr<M64, Imm>&);

        void exec(const Btc<R16, R16>&);
        void exec(const Btc<R16, Imm>&);
        void exec(const Btc<R32, R32>&);
        void exec(const Btc<R32, Imm>&);
        void exec(const Btc<R64, R64>&);
        void exec(const Btc<R64, Imm>&);
        void exec(const Btc<M16, R16>&);
        void exec(const Btc<M16, Imm>&);
        void exec(const Btc<M32, R32>&);
        void exec(const Btc<M32, Imm>&);
        void exec(const Btc<M64, R64>&);
        void exec(const Btc<M64, Imm>&);

        void exec(const Test<R8, R8>&);
        void exec(const Test<R8, Imm>&);
        void exec(const Test<M8, R8>&);
        void exec(const Test<M8, Imm>&);
        void exec(const Test<R16, R16>&);
        void exec(const Test<R16, Imm>&);
        void exec(const Test<M16, R16>&);
        void exec(const Test<M16, Imm>&);
        void exec(const Test<R32, R32>&);
        void exec(const Test<R32, Imm>&);
        void exec(const Test<M32, R32>&);
        void exec(const Test<M32, Imm>&);
        void exec(const Test<R64, R64>&);
        void exec(const Test<R64, Imm>&);
        void exec(const Test<M64, R64>&);
        void exec(const Test<M64, Imm>&);

        void exec(const Cmp<R8, R8>&);
        void exec(const Cmp<R8, Imm>&);
        void exec(const Cmp<R8, M8>&);
        void exec(const Cmp<M8, R8>&);
        void exec(const Cmp<M8, Imm>&);
        void exec(const Cmp<R16, R16>&);
        void exec(const Cmp<R16, Imm>&);
        void exec(const Cmp<R16, M16>&);
        void exec(const Cmp<M16, R16>&);
        void exec(const Cmp<M16, Imm>&);
        void exec(const Cmp<R32, R32>&);
        void exec(const Cmp<R32, Imm>&);
        void exec(const Cmp<R32, M32>&);
        void exec(const Cmp<M32, R32>&);
        void exec(const Cmp<M32, Imm>&);
        void exec(const Cmp<R64, R64>&);
        void exec(const Cmp<R64, Imm>&);
        void exec(const Cmp<R64, M64>&);
        void exec(const Cmp<M64, R64>&);
        void exec(const Cmp<M64, Imm>&);

        void exec(const Cmpxchg<R8, R8>&);
        void exec(const Cmpxchg<M8, R8>&);
        void exec(const Cmpxchg<R16, R16>&);
        void exec(const Cmpxchg<M16, R16>&);
        void exec(const Cmpxchg<R32, R32>&);
        void exec(const Cmpxchg<M32, R32>&);
        void exec(const Cmpxchg<R64, R64>&);
        void exec(const Cmpxchg<M64, R64>&);

        void exec(const Set<Cond::AE, R8>&);
        void exec(const Set<Cond::AE, M8>&);
        void exec(const Set<Cond::A, R8>&);
        void exec(const Set<Cond::A,M8>&);
        void exec(const Set<Cond::B, R8>&);
        void exec(const Set<Cond::B, M8>&);
        void exec(const Set<Cond::BE, R8>&);
        void exec(const Set<Cond::BE, M8>&);
        void exec(const Set<Cond::E, R8>&);
        void exec(const Set<Cond::E, M8>&);
        void exec(const Set<Cond::G, R8>&);
        void exec(const Set<Cond::G, M8>&);
        void exec(const Set<Cond::GE, R8>&);
        void exec(const Set<Cond::GE, M8>&);
        void exec(const Set<Cond::L, R8>&);
        void exec(const Set<Cond::L, M8>&);
        void exec(const Set<Cond::LE, R8>&);
        void exec(const Set<Cond::LE, M8>&);
        void exec(const Set<Cond::NE, R8>&);
        void exec(const Set<Cond::NE, M8>&);
        void exec(const Set<Cond::NO, R8>&);
        void exec(const Set<Cond::NO, M8>&);
        void exec(const Set<Cond::NS, R8>&);
        void exec(const Set<Cond::NS, M8>&);
        void exec(const Set<Cond::O, R8>&);
        void exec(const Set<Cond::O, M8>&);
        void exec(const Set<Cond::S, R8>&);
        void exec(const Set<Cond::S, M8>&);

        void exec(const Jmp<R32>&);
        void exec(const Jmp<R64>&);
        void exec(const Jmp<u32>&);
        void exec(const Jmp<M32>&);
        void exec(const Jmp<M64>&);
        void exec(const Jcc<Cond::NE>&);
        void exec(const Jcc<Cond::E>&);
        void exec(const Jcc<Cond::AE>&);
        void exec(const Jcc<Cond::BE>&);
        void exec(const Jcc<Cond::GE>&);
        void exec(const Jcc<Cond::LE>&);
        void exec(const Jcc<Cond::A>&);
        void exec(const Jcc<Cond::B>&);
        void exec(const Jcc<Cond::G>&);
        void exec(const Jcc<Cond::L>&);
        void exec(const Jcc<Cond::S>&);
        void exec(const Jcc<Cond::NS>&);
        void exec(const Jcc<Cond::O>&);
        void exec(const Jcc<Cond::NO>&);
        void exec(const Jcc<Cond::P>&);
        void exec(const Jcc<Cond::NP>&);

        void exec(const Bsr<R32, R32>&);
        void exec(const Bsr<R64, R64>&);

        void exec(const Bsf<R32, R32>&);
        void exec(const Bsf<R64, R64>&);

        void exec(const Cld&);
        void exec(const Std&);

        void exec(const Rep<Movs<Addr<Size::BYTE, B>, Addr<Size::BYTE, B>>>&);
        void exec(const Rep<Movs<Addr<Size::DWORD, B>, Addr<Size::DWORD, B>>>&);
        void exec(const Rep<Movs<M64, M64>>&);
        
        void exec(const Rep<Cmps<M8, M8>>&);
        
        void exec(const Rep<Stos<M32, R32>>&);
        void exec(const Rep<Stos<M64, R64>>&);

        void exec(const RepNZ<Scas<R8, Addr<Size::BYTE, B>>>&);

        void exec(const Cmov<Cond::AE, R32, R32>&);
        void exec(const Cmov<Cond::AE, R32, M32>&);
        void exec(const Cmov<Cond::A, R32, R32>&);
        void exec(const Cmov<Cond::A, R32, M32>&);
        void exec(const Cmov<Cond::BE, R32, R32>&);
        void exec(const Cmov<Cond::BE, R32, M32>&);
        void exec(const Cmov<Cond::B, R32, R32>&);
        void exec(const Cmov<Cond::B, R32, M32>&);
        void exec(const Cmov<Cond::E, R32, R32>&);
        void exec(const Cmov<Cond::E, R32, M32>&);
        void exec(const Cmov<Cond::GE, R32, R32>&);
        void exec(const Cmov<Cond::GE, R32, M32>&);
        void exec(const Cmov<Cond::G, R32, R32>&);
        void exec(const Cmov<Cond::G, R32, M32>&);
        void exec(const Cmov<Cond::LE, R32, R32>&);
        void exec(const Cmov<Cond::LE, R32, M32>&);
        void exec(const Cmov<Cond::L, R32, R32>&);
        void exec(const Cmov<Cond::L, R32, M32>&);
        void exec(const Cmov<Cond::NE, R32, R32>&);
        void exec(const Cmov<Cond::NE, R32, M32>&);
        void exec(const Cmov<Cond::NS, R32, R32>&);
        void exec(const Cmov<Cond::NS, R32, M32>&);
        void exec(const Cmov<Cond::S, R32, R32>&);
        void exec(const Cmov<Cond::S, R32, M32>&);
        void exec(const Cmov<Cond::AE, R64, R64>&);
        void exec(const Cmov<Cond::AE, R64, M64>&);
        void exec(const Cmov<Cond::A, R64, R64>&);
        void exec(const Cmov<Cond::A, R64, M64>&);
        void exec(const Cmov<Cond::BE, R64, R64>&);
        void exec(const Cmov<Cond::BE, R64, M64>&);
        void exec(const Cmov<Cond::B, R64, R64>&);
        void exec(const Cmov<Cond::B, R64, M64>&);
        void exec(const Cmov<Cond::E, R64, R64>&);
        void exec(const Cmov<Cond::E, R64, M64>&);
        void exec(const Cmov<Cond::GE, R64, R64>&);
        void exec(const Cmov<Cond::GE, R64, M64>&);
        void exec(const Cmov<Cond::G, R64, R64>&);
        void exec(const Cmov<Cond::G, R64, M64>&);
        void exec(const Cmov<Cond::LE, R64, R64>&);
        void exec(const Cmov<Cond::LE, R64, M64>&);
        void exec(const Cmov<Cond::L, R64, R64>&);
        void exec(const Cmov<Cond::L, R64, M64>&);
        void exec(const Cmov<Cond::NE, R64, R64>&);
        void exec(const Cmov<Cond::NE, R64, M64>&);
        void exec(const Cmov<Cond::NS, R64, R64>&);
        void exec(const Cmov<Cond::NS, R64, M64>&);
        void exec(const Cmov<Cond::S, R64, R64>&);
        void exec(const Cmov<Cond::S, R64, M64>&);

        void exec(const Cwde&);
        void exec(const Cdqe&);

        void exec(const Pxor<RSSE, RSSE>&);
        void exec(const Pxor<RSSE, MSSE>&);

        void exec(const Movaps<RSSE, RSSE>&);
        void exec(const Movaps<MSSE, RSSE>&);
        void exec(const Movaps<RSSE, MSSE>&);
        void exec(const Movaps<MSSE, MSSE>&);

        void exec(const Movd<RSSE, R32>&);
        void exec(const Movd<R32, RSSE>&);

        void exec(const Movq<RSSE, R64>&);
        void exec(const Movq<R64, RSSE>&);
        void exec(const Movq<RSSE, M64>&);
        void exec(const Movq<M64, RSSE>&);

        void exec(const Fldz&);
        void exec(const Fld1&);
        void exec(const Fld<ST>&);
        void exec(const Fld<M32>&);
        void exec(const Fld<M64>&);
        void exec(const Fld<M80>&);
        void exec(const Fild<M16>&);
        void exec(const Fild<M32>&);
        void exec(const Fild<M64>&);
        void exec(const Fstp<ST>&);
        void exec(const Fstp<M32>&);
        void exec(const Fstp<M64>&);
        void exec(const Fstp<M80>&);
        void exec(const Fistp<M16>&);
        void exec(const Fistp<M32>&);
        void exec(const Fistp<M64>&);
        void exec(const Fxch<ST>&);

        void exec(const Faddp<ST>&);
        void exec(const Fsubrp<ST>&);
        void exec(const Fmul1<M32>&);
        void exec(const Fmul1<M64>&);
        void exec(const Fdiv<ST, ST>&);
        void exec(const Fdivp<ST, ST>&);

        void exec(const Fcomi<ST>&);
        void exec(const Fucomi<ST>&);
        void exec(const Frndint&);

        void exec(const Fcmov<Cond::B, ST>&);
        void exec(const Fcmov<Cond::BE, ST>&);
        void exec(const Fcmov<Cond::E, ST>&);
        void exec(const Fcmov<Cond::NB, ST>&);
        void exec(const Fcmov<Cond::NBE, ST>&);
        void exec(const Fcmov<Cond::NE, ST>&);
        void exec(const Fcmov<Cond::NU, ST>&);
        void exec(const Fcmov<Cond::U, ST>&);

        void exec(const Fnstcw<M16>&);
        void exec(const Fldcw<M16>&);

        void exec(const Fnstsw<R16>&);
        void exec(const Fnstsw<M16>&);

        void exec(const Fnstenv<M224>&);
        void exec(const Fldenv<M224>&);

        void exec(const Movss<RSSE, M32>&);
        void exec(const Movss<M32, RSSE>&);

        void exec(const Movsd<RSSE, M64>&);
        void exec(const Movsd<M64, RSSE>&);
        
        void exec(const Addss<RSSE, RSSE>&);
        void exec(const Addss<RSSE, M32>&);
        void exec(const Addsd<RSSE, RSSE>&);
        void exec(const Addsd<RSSE, M64>&);

        void exec(const Subsd<RSSE, RSSE>&);
        void exec(const Subsd<RSSE, M64>&);
        void exec(const Subss<RSSE, RSSE>&);
        void exec(const Subss<RSSE, M32>&);

        void exec(const Mulsd<RSSE, RSSE>&);
        void exec(const Mulsd<RSSE, M64>&);

        void exec(const Divsd<RSSE, RSSE>&);
        void exec(const Divsd<RSSE, M64>&);

        void exec(const Comiss<RSSE, RSSE>&);
        void exec(const Comiss<RSSE, M32>&);
        void exec(const Comisd<RSSE, RSSE>&);
        void exec(const Comisd<RSSE, M64>&);
        void exec(const Ucomiss<RSSE, RSSE>&);
        void exec(const Ucomiss<RSSE, M32>&);
        void exec(const Ucomisd<RSSE, RSSE>&);
        void exec(const Ucomisd<RSSE, M64>&);

        void exec(const Cmpsd<RSSE, RSSE>&);
        void exec(const Cmpsd<RSSE, M64>&);

        void exec(const Cvtsi2sd<RSSE, R32>&);
        void exec(const Cvtsi2sd<RSSE, M32>&);
        void exec(const Cvtsi2sd<RSSE, R64>&);
        void exec(const Cvtsi2sd<RSSE, M64>&);
        void exec(const Cvtss2sd<RSSE, RSSE>&);
        void exec(const Cvtss2sd<RSSE, M32>&);

        void exec(const Cvttsd2si<R32, RSSE>&);
        void exec(const Cvttsd2si<R32, M64>&);
        void exec(const Cvttsd2si<R64, RSSE>&);
        void exec(const Cvttsd2si<R64, M64>&);

        void exec(const Pand<RSSE, RSSE>&);
        void exec(const Pand<RSSE, MSSE>&);
        void exec(const Pandn<RSSE, RSSE>&);
        void exec(const Pandn<RSSE, MSSE>&);
        void exec(const Por<RSSE, RSSE>&);
        void exec(const Por<RSSE, MSSE>&);

        void exec(const Andpd<RSSE, RSSE>&);
        void exec(const Andnpd<RSSE, RSSE>&);
        void exec(const Orpd<RSSE, RSSE>&);
        void exec(const Xorpd<RSSE, RSSE>&);

        void exec(const Movlps<RSSE, M64>&);
        void exec(const Movhps<RSSE, M64>&);

        void exec(const Punpcklbw<RSSE, RSSE>&);
        void exec(const Punpcklwd<RSSE, RSSE>&);
        void exec(const Punpckldq<RSSE, RSSE>&);
        void exec(const Punpcklqdq<RSSE, RSSE>&);

        void exec(const Punpckhbw<RSSE, RSSE>&);
        void exec(const Punpckhwd<RSSE, RSSE>&);
        void exec(const Punpckhdq<RSSE, RSSE>&);
        void exec(const Punpckhqdq<RSSE, RSSE>&);

        void exec(const Pshufb<RSSE, RSSE>&);
        void exec(const Pshufb<RSSE, MSSE>&);
        void exec(const Pshufd<RSSE, RSSE, Imm>&);
        void exec(const Pshufd<RSSE, MSSE, Imm>&);

        void exec(const Pcmpeqb<RSSE, RSSE>&);
        void exec(const Pcmpeqb<RSSE, MSSE>&);
        void exec(const Pcmpeqw<RSSE, RSSE>&);
        void exec(const Pcmpeqw<RSSE, MSSE>&);
        void exec(const Pcmpeqd<RSSE, RSSE>&);
        void exec(const Pcmpeqd<RSSE, MSSE>&);
        void exec(const Pcmpeqq<RSSE, RSSE>&);
        void exec(const Pcmpeqq<RSSE, MSSE>&);

        void exec(const Pcmpgtb<RSSE, RSSE>&);
        void exec(const Pcmpgtb<RSSE, MSSE>&);
        void exec(const Pcmpgtw<RSSE, RSSE>&);
        void exec(const Pcmpgtw<RSSE, MSSE>&);
        void exec(const Pcmpgtd<RSSE, RSSE>&);
        void exec(const Pcmpgtd<RSSE, MSSE>&);
        void exec(const Pcmpgtq<RSSE, RSSE>&);
        void exec(const Pcmpgtq<RSSE, MSSE>&);

        void exec(const Pmovmskb<R32, RSSE>&);

        void exec(const Paddb<RSSE, RSSE>&);
        void exec(const Paddb<RSSE, MSSE>&);
        void exec(const Paddw<RSSE, RSSE>&);
        void exec(const Paddw<RSSE, MSSE>&);
        void exec(const Paddd<RSSE, RSSE>&);
        void exec(const Paddd<RSSE, MSSE>&);
        void exec(const Paddq<RSSE, RSSE>&);
        void exec(const Paddq<RSSE, MSSE>&);

        void exec(const Psubb<RSSE, RSSE>&);
        void exec(const Psubb<RSSE, MSSE>&);
        void exec(const Psubw<RSSE, RSSE>&);
        void exec(const Psubw<RSSE, MSSE>&);
        void exec(const Psubd<RSSE, RSSE>&);
        void exec(const Psubd<RSSE, MSSE>&);
        void exec(const Psubq<RSSE, RSSE>&);
        void exec(const Psubq<RSSE, MSSE>&);

        void exec(const Pminub<RSSE, RSSE>&);
        void exec(const Pminub<RSSE, MSSE>&);

        void exec(const Ptest<RSSE, RSSE>&);
        void exec(const Ptest<RSSE, MSSE>&);

        void exec(const Psllw<RSSE, Imm>&);
        void exec(const Pslld<RSSE, Imm>&);
        void exec(const Psllq<RSSE, Imm>&);
        void exec(const Psrlw<RSSE, Imm>&);
        void exec(const Psrld<RSSE, Imm>&);
        void exec(const Psrlq<RSSE, Imm>&);

        void exec(const Pslldq<RSSE, Imm>&);
        void exec(const Psrldq<RSSE, Imm>&);
        
        void exec(const Pcmpistri<RSSE, RSSE, Imm>&);
        void exec(const Pcmpistri<RSSE, MSSE, Imm>&);
        
        void exec(const Rdtsc&);

        void exec(const Cpuid&);
        void exec(const Xgetbv&);

        void exec(const Fxsave<M64>&);
        void exec(const Fxrstor<M64>&);

        void exec(const Fwait&);

        void exec(const Rdpkru&);
        void exec(const Wrpkru&);

        std::string functionName(const X86Instruction& instruction) const;

    };

}

#endif
