...
namespace kernel {
    ...
    static u64 loadElf(x64::Mmu* mmu, Auxiliary* auxiliary, const std::string& filepath, bool mainProgram) {
        ...
        x64::verify(!!elf, [&]() { fmt::print("Failed to load elf {}\n", filepath); });
        x64::verify(elf->archClass() == elf::Class::B64, "elf must be 64-bit");
        std::unique_ptr<elf::Elf64> elf64;
        elf64.reset(static_cast<elf::Elf64*>(elf.release()));

        u64 elfOffset = [&]() -> u64 {
            x64::verify(elf64->type() == elf::Type::ET_DYN || elf64->type() == elf::Type::ET_EXEC, "elf must be ET_DYN or ET_EXEC");
        ...
        elf64->forAllProgramHeaders([&](const elf::ProgramHeader64& header) {
            if(header.type() != elf::ProgramHeaderType::PT_LOAD) return;
            x64::verify(header.alignment() % x64::Mmu::PAGE_SIZE == 0);
    ...
    static u64 setupMemory(x64::Mmu* mmu, Auxiliary* auxiliary) {
        {
            ...
            x64::verify(!!auxiliary, "no auxiliary...");
    ...
    static void pushProgramArguments(x64::Mmu* mmu, x64::VM* vm, const std::string& programFilePath, const std::vector<std::string>& arguments, const std::vector<std::string>& environmentVariables, const Auxiliary& auxiliary) {
        ...
        auto writeArgument = [&](const std::string& s) -> x64::Ptr8 {
            ...
            x64::verify(buffer.back() == 0x0, "string is not null-terminated");
