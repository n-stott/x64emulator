...
namespace kernel::gnulinux {
    ...
    std::unique_ptr<SharedMemorySegment> SharedMemorySegment::tryCreate(int mode, size_t size) {
        if(mode & (~0777)) return {};
        int id = ::shmget(IPC_PRIVATE, size, IPC_CREAT | mode);
        if(id < 0) return {};
        return std::unique_ptr<SharedMemorySegment>(new SharedMemorySegment(id, mode, size));
    }

    SharedMemorySegment::SharedMemorySegment(int id, int mode, size_t size) :
            id_(id),
            mode_(mode),
            size_(size) {
        // we are good !
    ...
    ErrnoOr<u64> SharedMemorySegment::attach(x64::Mmu* mmu, bool readonly, bool executable) {
        ...
        auto addr = mmu->mmap(0, size_, prot, flags);
        if(!addr) return ErrnoOr<u64>(-ENOMEM);
        void* ret = ::shmat(id_, (void*)(mmu->base() + addr.value()), SHM_REMAP);
        if(ret == (void*)(-1)) {
    ...
    int SharedMemorySegment::detach(x64::Mmu* mmu) {
        ...
        int ret = ::shmdt((void*)(mmu->base() + attachedAddress_.value()));
