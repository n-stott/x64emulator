...
namespace x64 {
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeAdd(const cs_insn& insn) {
        ...
        auto m64dst = asMemory64(dst);
        auto m64src = asMemory64(src);
        if(r8dst && r8src) return make_wrapper<Add<R8, R8>>(insn.address, r8dst.value(), r8src.value());
        if(r8dst && immsrc) return make_wrapper<Add<R8, Imm>>(insn.address, r8dst.value(), immsrc.value());
        if(r8dst && m8src) return make_wrapper<Add<R8, M8>>(insn.address, r8dst.value(), m8src.value());
        if(m8dst && r8src) return make_wrapper<Add<M8, R8>>(insn.address, m8dst.value(), r8src.value());
        if(m8dst && immsrc) return make_wrapper<Add<M8, Imm>>(insn.address, m8dst.value(), immsrc.value());
        if(r16dst && r16src) return make_wrapper<Add<R16, R16>>(insn.address, r16dst.value(), r16src.value());
        if(r16dst && immsrc) return make_wrapper<Add<R16, Imm>>(insn.address, r16dst.value(), immsrc.value());
        if(r16dst && m16src) return make_wrapper<Add<R16, M16>>(insn.address, r16dst.value(), m16src.value());
        if(m16dst && r16src) return make_wrapper<Add<M16, R16>>(insn.address, m16dst.value(), r16src.value());
        if(m16dst && immsrc) return make_wrapper<Add<M16, Imm>>(insn.address, m16dst.value(), immsrc.value());
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeSub(const cs_insn& insn) {
        ...
        auto m64dst = asMemory64(dst);
        auto m64src = asMemory64(src);
        if(r8dst && r8src) return make_wrapper<Sub<R8, R8>>(insn.address, r8dst.value(), r8src.value());
        if(r8dst && immsrc) return make_wrapper<Sub<R8, Imm>>(insn.address, r8dst.value(), immsrc.value());
        if(r8dst && m8src) return make_wrapper<Sub<R8, M8>>(insn.address, r8dst.value(), m8src.value());
        if(m8dst && r8src) return make_wrapper<Sub<M8, R8>>(insn.address, m8dst.value(), r8src.value());
        if(m8dst && immsrc) return make_wrapper<Sub<M8, Imm>>(insn.address, m8dst.value(), immsrc.value());
        if(r16dst && r16src) return make_wrapper<Sub<R16, R16>>(insn.address, r16dst.value(), r16src.value());
        if(r16dst && immsrc) return make_wrapper<Sub<R16, Imm>>(insn.address, r16dst.value(), immsrc.value());
        if(r16dst && m16src) return make_wrapper<Sub<R16, M16>>(insn.address, r16dst.value(), m16src.value());
        if(m16dst && r16src) return make_wrapper<Sub<M16, R16>>(insn.address, m16dst.value(), r16src.value());
        if(m16dst && immsrc) return make_wrapper<Sub<M16, Imm>>(insn.address, m16dst.value(), immsrc.value());
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeSbb(const cs_insn& insn) {
        ...
        auto m64dst = asMemory64(dst);
        auto m64src = asMemory64(src);
        if(r8dst && immsrc) return make_wrapper<Sbb<R8, Imm>>(insn.address, r8dst.value(), immsrc.value().immediate);
        if(r8dst && r8src) return make_wrapper<Sbb<R8, R8>>(insn.address, r8dst.value(), r8src.value());
        if(r8dst && m8src) return make_wrapper<Sbb<R8, M8>>(insn.address, r8dst.value(), m8src.value());
        if(m8dst && r8src) return make_wrapper<Sbb<M8, R8>>(insn.address, m8dst.value(), r8src.value());
        if(m8dst && immsrc) return make_wrapper<Sbb<M8, Imm>>(insn.address, m8dst.value(), immsrc.value());
        if(r16dst && immsrc) return make_wrapper<Sbb<R16, Imm>>(insn.address, r16dst.value(), immsrc.value().immediate);
        if(r16dst && r16src) return make_wrapper<Sbb<R16, R16>>(insn.address, r16dst.value(), r16src.value());
        if(r16dst && m16src) return make_wrapper<Sbb<R16, M16>>(insn.address, r16dst.value(), m16src.value());
        if(m16dst && r16src) return make_wrapper<Sbb<M16, R16>>(insn.address, m16dst.value(), r16src.value());
        if(m16dst && immsrc) return make_wrapper<Sbb<M16, Imm>>(insn.address, m16dst.value(), immsrc.value());
        if(r32dst && immsrc) return make_wrapper<Sbb<R32, Imm>>(insn.address, r32dst.value(), immsrc.value().immediate);
        ...
        if(r64dst && immsrc) return make_wrapper<Sbb<R64, Imm>>(insn.address, r64dst.value(), immsrc.value().immediate);
        if(r64dst && r64src) return make_wrapper<Sbb<R64, R64>>(insn.address, r64dst.value(), r64src.value());
        if(r64dst && m64src) return make_wrapper<Sbb<R64, M64>>(insn.address, r64dst.value(), m64src.value());
        if(m64dst && r64src) return make_wrapper<Sbb<M64, R64>>(insn.address, m64dst.value(), r64src.value());
        if(m64dst && immsrc) return make_wrapper<Sbb<M64, Imm>>(insn.address, m64dst.value(), immsrc.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeNeg(const cs_insn& insn) {
        ...
        if(r8dst) return make_wrapper<Neg<R8>>(insn.address, r8dst.value());
        if(m8dst) return make_wrapper<Neg<M8>>(insn.address, m8dst.value());
        if(r16dst) return make_wrapper<Neg<R16>>(insn.address, r16dst.value());
        if(m16dst) return make_wrapper<Neg<M16>>(insn.address, m16dst.value());
        ...
        if(r64dst) return make_wrapper<Neg<R64>>(insn.address, r64dst.value());
        if(m64dst) return make_wrapper<Neg<M64>>(insn.address, m64dst.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeIdiv(const cs_insn& insn) {
        ...
        if(r64dst) return make_wrapper<Idiv<R64>>(insn.address, r64dst.value());
        if(m64dst) return make_wrapper<Idiv<M64>>(insn.address, m64dst.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeXchg(const cs_insn& insn) {
        ...
        if(r64dst && r64src) return make_wrapper<Xchg<R64, R64>>(insn.address, r64dst.value(), r64src.value());
        if(m64dst && r64src) return make_wrapper<Xchg<M64, R64>>(insn.address, m64dst.value(), r64src.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeShr(const cs_insn& insn) {
        ...
        auto r8src = asRegister8(src);
        auto immsrc = asImmediate(src);
        if(r8dst && r8src) return make_wrapper<Shr<R8, R8>>(insn.address, r8dst.value(), r8src.value());
        if(r8dst && immsrc) return make_wrapper<Shr<R8, Imm>>(insn.address, r8dst.value(), immsrc.value());
        if(r16dst && r8src) return make_wrapper<Shr<R16, R8>>(insn.address, r16dst.value(), r8src.value());
        ...
        if(m32dst && r8src) return make_wrapper<Shr<M32, R8>>(insn.address, m32dst.value(), r8src.value());
        if(r32dst && immsrc) return make_wrapper<Shr<R32, Imm>>(insn.address, r32dst.value(), immsrc.value());
        if(m32dst && immsrc) return make_wrapper<Shr<M32, Imm>>(insn.address, m32dst.value(), immsrc.value());
        if(r64dst && r8src) return make_wrapper<Shr<R64, R8>>(insn.address, r64dst.value(), r8src.value());
        if(m64dst && r8src) return make_wrapper<Shr<M64, R8>>(insn.address, m64dst.value(), r8src.value());
        if(r64dst && immsrc) return make_wrapper<Shr<R64, Imm>>(insn.address, r64dst.value(), immsrc.value());
        if(m64dst && immsrc) return make_wrapper<Shr<M64, Imm>>(insn.address, m64dst.value(), immsrc.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeShl(const cs_insn& insn) {
        ...
        auto m32dst = asMemory32(dst);
        auto m64dst = asMemory64(dst);
        if(r8dst && r8src) return make_wrapper<Shl<R8, R8>>(insn.address, r8dst.value(), r8src.value());
        if(r8dst && immsrc) return make_wrapper<Shl<R8, Imm>>(insn.address, r8dst.value(), immsrc.value());
        if(r16dst && r8src) return make_wrapper<Shl<R16, R8>>(insn.address, r16dst.value(), r8src.value());
        if(r16dst && immsrc) return make_wrapper<Shl<R16, Imm>>(insn.address, r16dst.value(), immsrc.value());
        ...
        if(m32dst && r8src) return make_wrapper<Shl<M32, R8>>(insn.address, m32dst.value(), r8src.value());
        if(r64dst && r8src) return make_wrapper<Shl<R64, R8>>(insn.address, r64dst.value(), r8src.value());
        if(m64dst && r8src) return make_wrapper<Shl<M64, R8>>(insn.address, m64dst.value(), r8src.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeShrd(const cs_insn& insn) {
        ...
        if(r64dst && r64src1 && r8src2) return make_wrapper<Shrd<R64, R64, R8>>(insn.address, r64dst.value(), r64src1.value(), r8src2.value());
        if(r64dst && r64src1 && immsrc2) return make_wrapper<Shrd<R64, R64, Imm>>(insn.address, r64dst.value(), r64src1.value(), immsrc2.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeShld(const cs_insn& insn) {
        ...
        if(r64dst && r64src1 && r8src2) return make_wrapper<Shld<R64, R64, R8>>(insn.address, r64dst.value(), r64src1.value(), r8src2.value());
        if(r64dst && r64src1 && immsrc2) return make_wrapper<Shld<R64, R64, Imm>>(insn.address, r64dst.value(), r64src1.value(), immsrc2.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeRol(const cs_insn& insn) {
        ...
        auto r8src = asRegister8(src);
        auto immsrc = asImmediate(src);
        if(r8dst && r8src) return make_wrapper<Rol<R8, R8>>(insn.address, r8dst.value(), r8src.value());
        if(r8dst && immsrc) return make_wrapper<Rol<R8, Imm>>(insn.address, r8dst.value(), immsrc.value());
        if(m8dst && immsrc) return make_wrapper<Rol<M8, Imm>>(insn.address, m8dst.value(), immsrc.value());
        if(r16dst && r8src) return make_wrapper<Rol<R16, R8>>(insn.address, r16dst.value(), r8src.value());
        if(r16dst && immsrc) return make_wrapper<Rol<R16, Imm>>(insn.address, r16dst.value(), immsrc.value());
        if(m16dst && immsrc) return make_wrapper<Rol<M16, Imm>>(insn.address, m16dst.value(), immsrc.value());
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeRor(const cs_insn& insn) {
        ...
        auto r8src = asRegister8(src);
        auto immsrc = asImmediate(src);
        if(r8dst && r8src) return make_wrapper<Ror<R8, R8>>(insn.address, r8dst.value(), r8src.value());
        if(r8dst && immsrc) return make_wrapper<Ror<R8, Imm>>(insn.address, r8dst.value(), immsrc.value());
        if(m8dst && immsrc) return make_wrapper<Ror<M8, Imm>>(insn.address, m8dst.value(), immsrc.value());
        if(r16dst && r8src) return make_wrapper<Ror<R16, R8>>(insn.address, r16dst.value(), r8src.value());
        if(r16dst && immsrc) return make_wrapper<Ror<R16, Imm>>(insn.address, r16dst.value(), immsrc.value());
        if(m16dst && immsrc) return make_wrapper<Ror<M16, Imm>>(insn.address, m16dst.value(), immsrc.value());
        if(r32dst && r8src) return make_wrapper<Ror<R32, R8>>(insn.address, r32dst.value(), r8src.value());
        if(r32dst && immsrc) return make_wrapper<Ror<R32, Imm>>(insn.address, r32dst.value(), immsrc.value());
        if(m32dst && immsrc) return make_wrapper<Ror<M32, Imm>>(insn.address, m32dst.value(), immsrc.value());
        if(r64dst && r8src) return make_wrapper<Ror<R64, R8>>(insn.address, r64dst.value(), r8src.value());
        if(r64dst && immsrc) return make_wrapper<Ror<R64, Imm>>(insn.address, r64dst.value(), immsrc.value());
        if(m64dst && immsrc) return make_wrapper<Ror<M64, Imm>>(insn.address, m64dst.value(), immsrc.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeTzcnt(const cs_insn& insn) {
        ...
        auto r64src = asRegister64(src);
        auto m64src = asMemory64(src);
        if(r16dst && r16src) return make_wrapper<Tzcnt<R16, R16>>(insn.address, r16dst.value(), r16src.value());
        if(r16dst && m16src) return make_wrapper<Tzcnt<R16, M16>>(insn.address, r16dst.value(), m16src.value());
        if(r32dst && r32src) return make_wrapper<Tzcnt<R32, R32>>(insn.address, r32dst.value(), r32src.value());
        if(r32dst && m32src) return make_wrapper<Tzcnt<R32, M32>>(insn.address, r32dst.value(), m32src.value());
        if(r64dst && r64src) return make_wrapper<Tzcnt<R64, R64>>(insn.address, r64dst.value(), r64src.value());
        if(r64dst && m64src) return make_wrapper<Tzcnt<R64, M64>>(insn.address, r64dst.value(), m64src.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeBt(const cs_insn& insn) {
        ...
        if(r16src1 && r16src2) return make_wrapper<Bt<R16, R16>>(insn.address, r16src1.value(), r16src2.value());
        if(r16src1 && immsrc2) return make_wrapper<Bt<R16, Imm>>(insn.address, r16src1.value(), immsrc2.value());
        if(r32src1 && r32src2) return make_wrapper<Bt<R32, R32>>(insn.address, r32src1.value(), r32src2.value());
        if(r32src1 && immsrc2) return make_wrapper<Bt<R32, Imm>>(insn.address, r32src1.value(), immsrc2.value());
        if(r64src1 && r64src2) return make_wrapper<Bt<R64, R64>>(insn.address, r64src1.value(), r64src2.value());
        if(r64src1 && immsrc2) return make_wrapper<Bt<R64, Imm>>(insn.address, r64src1.value(), immsrc2.value());
        if(m16src1 && r16src2) return make_wrapper<Bt<M16, R16>>(insn.address, m16src1.value(), r16src2.value());
        if(m16src1 && immsrc2) return make_wrapper<Bt<M16, Imm>>(insn.address, m16src1.value(), immsrc2.value());
        if(m32src1 && r32src2) return make_wrapper<Bt<M32, R32>>(insn.address, m32src1.value(), r32src2.value());
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeBtc(const cs_insn& insn) {
        ...
        auto r64src2 = asRegister64(offset);
        auto immsrc2 = asImmediate(offset);
        if(r16src1 && r16src2) return make_wrapper<Btc<R16, R16>>(insn.address, r16src1.value(), r16src2.value());
        if(r16src1 && immsrc2) return make_wrapper<Btc<R16, Imm>>(insn.address, r16src1.value(), immsrc2.value());
        if(r32src1 && r32src2) return make_wrapper<Btc<R32, R32>>(insn.address, r32src1.value(), r32src2.value());
        if(r32src1 && immsrc2) return make_wrapper<Btc<R32, Imm>>(insn.address, r32src1.value(), immsrc2.value());
        if(r64src1 && r64src2) return make_wrapper<Btc<R64, R64>>(insn.address, r64src1.value(), r64src2.value());
        if(r64src1 && immsrc2) return make_wrapper<Btc<R64, Imm>>(insn.address, r64src1.value(), immsrc2.value());
        if(m16src1 && r16src2) return make_wrapper<Btc<M16, R16>>(insn.address, m16src1.value(), r16src2.value());
        if(m16src1 && immsrc2) return make_wrapper<Btc<M16, Imm>>(insn.address, m16src1.value(), immsrc2.value());
        if(m32src1 && r32src2) return make_wrapper<Btc<M32, R32>>(insn.address, m32src1.value(), r32src2.value());
        if(m32src1 && immsrc2) return make_wrapper<Btc<M32, Imm>>(insn.address, m32src1.value(), immsrc2.value());
        if(m64src1 && r64src2) return make_wrapper<Btc<M64, R64>>(insn.address, m64src1.value(), r64src2.value());
        if(m64src1 && immsrc2) return make_wrapper<Btc<M64, Imm>>(insn.address, m64src1.value(), immsrc2.value());
        return make_failed(insn);
    }
    ...
    std::unique_ptr<X86Instruction> CapstoneWrapper::makeCmp(const cs_insn& insn) {
        ...
        if(r16src1 && m16src2) return make_wrapper<Cmp<R16, M16>>(insn.address, r16src1.value(), m16src2.value());
