...
void testBt32(R32 dst, R32 src) {
    Assembler assembler;
    assembler.bt(dst, src);
    ...
    verify(ins.insn() == Insn::BT_RM32_R32);
...
// void testBsr64(R64 dst, R64 src) {
//     Assembler assembler;
...
//     verify(disassembly.instructions.size() == 1);
//     const auto& ins = disassembly.instructions[0];
//     verify(ins.insn() == Insn::Bsr_RM64_RM64);
...
// void testBsr64(R64 dst, i32 imm) {
//     Assembler assembler;
//     assembler.Bsr(dst, imm);
//     std::vector<u8> code = assembler.code();
//     auto disassembly = CapstoneWrapper::disassembleRange(code.data(), code.size(), 0x0);
//     verify(disassembly.instructions.size() == 1);
//     const auto& ins = disassembly.instructions[0];
//     verify(ins.insn() == Insn::Bsr_RM64_IMM);
//     RM64 disdst = ins.op0<RM64>();
