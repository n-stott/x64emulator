...
void testXor32(R32 dst, R32 src) {
    Assembler assembler;
    assembler.xor_(dst, src);
    ...
    verify(ins.insn() == Insn::XOR_RM32_RM32);
...
void testXor64(R64 dst, R64 src) {
    Assembler assembler;
    assembler.xor_(dst, src);
    std::vector<u8> code = assembler.code();
    auto disassembly = CapstoneWrapper::disassembleRange(code.data(), code.size(), 0x0);
    verify(disassembly.instructions.size() == 1);
    const auto& ins = disassembly.instructions[0];
    verify(ins.insn() == Insn::XOR_RM64_RM64);
